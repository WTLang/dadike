...
@section('content_01')
		...
		<div id="content">
	        ...
	        @endif
				<form action="/admin/admin/{{ $data[0]->id }}" method="post" class="form-horizontal" />
					{{ csrf_field() }}
					{{ method_field('PUT') }}
					<div class="control-group">
						<label class="control-label">用户名</label>
						<div class="controls">
							<input type="text" value="{{ $data[0]->admin_name }}" name = "admin_name"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">管理员邮箱</label>
						<div class="controls">
							<input type="text" name="admin_email" placeholder="输入邮箱" value="{{ $data[0]->admin_email }}">
						</div>
					</div>
						<div class="control-group">
